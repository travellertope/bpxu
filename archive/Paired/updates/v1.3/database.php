...
if (isset($_POST["btn_admin"])) {
    ...
    if (mysqli_connect_errno()) {
        ...
        mysqli_query($connection, "ALTER TABLE `settings` ADD `site_mode` VARCHAR(155) NULL DEFAULT 'light' AFTER `front_layout`;");

        mysqli_query($connection, "ALTER TABLE `settings` ADD `booking_date_type` VARCHAR(255) NULL DEFAULT 'slot' AFTER `time_zone`;");

        mysqli_query($connection, "ALTER TABLE `users` ADD `referral_id` VARCHAR(255) NULL AFTER `role`, ADD `referral_earn` VARCHAR(255) NULL DEFAULT '0' AFTER `referral_id`;");
        mysqli_query($connection, "ALTER TABLE `referrals` ADD `user_id` INT NULL DEFAULT '0' AFTER `referrar_id`, ADD `status` INT NULL DEFAULT '0' AFTER `user_id`;");

        mysqli_query($connection, "UPDATE `lang_values` SET `label` = 'Earn commission for their first session booking payments!', `english` = 'Earn commission for their first session booking payments!' WHERE `lang_values`.`keyword`='earn-commission-for-their-first-subscription-plan-payments';");


        mysqli_query($connection, "INSERT INTO `lang_values` (`type`, `label`, `keyword`, `english`) VALUES
        ('user', 'API Usage', 'api-usage', 'API Usage'),
        ('user', 'Utilize the Admin Zoom API to manage all users zoom meetings.', 'utilize-the-admin-zoom-api', 'Utilize the Admin Zoom API to manage all users zoom meetings.'),
        ('user', 'Allow users to manage their Zoom meetings via their individual zoom API', 'allow-users-to-manage-their-zoom-api', 'Allow users to manage their Zoom meetings via their individual zoom API'),
        ('user', 'Site Color Mode', 'site-color-mode', 'Site Color Mode'),
        ('user', 'Saturday', 'saturday', 'Saturday');");
