...
class Coupon extends Home_Controller {
    ...
    public function index()
    {
        ...
        $data['discounts'] = $this->admin_model->select_by_user('coupons');
        $data['main_content'] = $this->load->view('admin/user/coupon',$data,TRUE);
    ...
    {	
        ...
        {   
            ...
            if ($this->form_validation->run() === false) {
                ...
                if (empty($this->input->post('once_per_mentee'))) {
                    $once_per_customer = 0;
                }else {
                    $once_per_customer = $this->input->post('once_per_mentee');
                }
                $data=array(
                    'user_id' => user()->id,
                    ...
                    'once_per_mentee' => $once_per_customer,
    ...
    public function used_coupon($coupon_id)
    {
        $data = array();
        $data['page_title'] = 'Used Coupon';      
        $data['page'] = 'Coupon';   
        $data['coupon'] = FALSE;
        $data['used_coupons'] = $this->admin_model->get_all_used_coupons_by_mentee($coupon_id);
        $data['main_content'] = $this->load->view('admin/user/used_coupon',$data,TRUE);
        $this->load->view('admin/index',$data);
    }
