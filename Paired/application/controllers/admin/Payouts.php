...
class Payouts extends Home_Controller {
    ...
    public function update_settings()
    {
        ...
        if(get_user_info() == TRUE){
          ...
          if ($commission_rate < 1 || $commission_rate > 20) {
    ...
    public function complete($id) 
    {
        ...
        $user = $this->admin_model->get_by_id($payput->user_id,'users');
        ...
        $data = $this->security->xss_clean($data);
        $this->admin_model->edit_option_md5($data, $user->id,'users');
        ...
        if($_POST){   
            ...
            if ($check_balance == true) {
                ...
                $notify = array(
                    ...
                    'action_id' =>user()->id,
                    'content_id' => 0,
                    'text' => user()->name." sent a withdrawl request of amount ".$amount/100,
                    'noti_type' => 8,
                    'noti_time' => my_date_now()
                );
                $notify = $this->security->xss_clean($notify);
