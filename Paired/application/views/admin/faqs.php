              ...
              <form method="post" enctype="multipart/form-data" class="validate-form" action="<?php echo base_url('admin/faq/add')?>" role="form" novalidate>
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="title" value="<?php if(isset($faq[0]['title'])) {echo html_escape($faq[0]['title']);} ?>" <?php echo html_escape($required); ?>>
                    </div>
                    <div class="form-group">
                        <label><?php echo trans('details') ?></label>
                        <textarea id="ckEditor" class="form-control" name="details" rows="6"><?php if(isset($faq[0]['details'])) {echo html_escape($faq[0]['details']);} ?></textarea>
                    </div>
                </div>

                <div class="card-footer">
                  <input type="hidden" name="id" value="<?php if(isset($faq[0]['id'])) {echo html_escape($faq[0]['id']);} ?>">
                  <!-- csrf token -->
