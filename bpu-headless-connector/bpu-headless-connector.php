...
class BPU_Headless_Connector {
    ...
    public function register_api_routes() {
        ...
        register_rest_route( $this->namespace, '/admin/birthdays', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array( $this, 'admin_get_birthdays_dashboard' ),
            'permission_callback' => array( $this, 'check_admin_jwt_auth' ),
        ) );

        register_rest_route( $this->namespace, '/admin/birthdays/send', array(
            'methods'             => WP_REST_Server::CREATABLE,
...
' );
    ...
    /**
     * GET /admin/birthdays — members with a birthday in the given month, filterable
     * by email status (sent / missed / today / upcoming) and by name/email search.
     */
    public function admin_get_birthdays_dashboard( WP_REST_Request $request ) {
        $year   = (int) date( 'Y' );
        $month  = max( 1, min( 12, intval( $request->get_param( 'month' ) ?: date( 'n' ) ) ) );
        $status = sanitize_text_field( $request->get_param( 'status' ) ?: 'all' );
        $search = sanitize_text_field( $request->get_param( 'search' ) ?: '' );
        ...
        $summary = array(
            'today'    => count( array_filter( $month_rows, fn( $r ) => $r['is_today'] ) ),
            'sent'     => count( array_filter( $month_rows, fn( $r ) => $r['occurred'] && $r['sent_this_year'] ) ),
            'missed'   => count( array_filter( $month_rows, fn( $r ) => $r['occurred'] && ! $r['sent_this_year'] && ! $r['is_today'] ) ),
            'upcoming' => count( array_filter( $month_rows, fn( $r ) => ! $r['occurred'] ) ),
        );
    ...
    /**
     * POST /admin/birthdays/send — manually send (or resend) the birthday email to one member.
     */
    public function admin_send_birthday_email( WP_REST_Request $request ) {
        $user_id = intval( $request->get_param( 'user_id' ) );
        $user    = get_userdata( $user_id );
        ...
        if ( ! $this->send_single_birthday_email( $user ) ) {
            return new WP_Error( 'bpu_send_failed', __( 'Email failed to send.', 'bpu' ), array( 'status' => 500 ) );
        }
