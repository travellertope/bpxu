...
export async function GET() {
    ...
    try {
        const res = await fetch(`${WP}/wp-json/bpu/v1/paired/mentor/education`, {
...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        if (!res.ok) {
            return NextResponse.json({ error: data.message || 'Failed to create education entry.' }, { status: res.status });
        }
