...
export async function GET(
    ...
    try {
        const res = await fetch(
            `${WP}/wp-json/bpu/v1/paired/admin/newsletter/lists/${id}/subscribers${qs ? `?${qs}` : ''}`,
...
export async function POST(
    ...
    try {
        const res = await fetch(`${WP}/wp-json/bpu/v1/paired/admin/newsletter/lists/${id}/subscribers`, {
            method: 'POST',
            cache: 'no-store',
            headers: {
