 ...
 * contact-request form (no sensitive personal data). Like the contact
 * form, it's stored via the shared bpu_form_submission post type as the
 * durable record, with the notification email as best-effort.
 */
...
function bpu_ie_partnership_form_notice() {
    ...
    if ( 'success' === $status ) {
        echo '<div class="form-notice success">' . esc_html( bpu_ie_option( 'partnership_success_message', 'Thanks — our team will be in touch to schedule a call.' ) ) . '</div>';
    } elseif ( 'error' === $status ) {
        echo '<div class="form-notice error">' . esc_html( bpu_ie_option( 'partnership_error_message', 'Something went wrong submitting the form. Please check the required fields and try again.' ) ) . '</div>';
...
function bpu_ie_handle_partnership_submission() {
    ...
    if ( ! bpu_ie_verify_captcha() ) {
        wp_safe_redirect( add_query_arg( 'bpu_pf', 'captcha', $redirect_url ) );
        exit;
    }
    ...
    $submission_id = bpu_ie_store_form_submission( 'partnership', $org_name, array(
    ...
    wp_mail( bpu_ie_notification_recipients(), $subject, $body, $headers );
